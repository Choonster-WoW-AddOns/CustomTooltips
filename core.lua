...
local TOOLTIPS = {
	...
	Example = {
		...
		body = {
			{ "This is an example tooltip. The text will be in red.",                               1.0, 0.0, 0.0 },
	...
	ExampleTwo = {
		...
		body = {
			{ "This is a second example tooltip; it doesn't have any linebreaks in it. It uses the default yellow colour." }
		}
	},
	...
	ExampleThreeWithSyntaxError = {
		...
		body = {
			{ "This is an erroneous tooltip. If used, it should display the syntax error tooltip.", 2.0 }
		}
	},
...
--- @class TooltipLine
--- @field [1] string Tooltip text
--- @field [2] number? Red
--- @field [3] number? Green
--- @field [4] number? Blue

--- @alias TooltipBody TooltipLine[]

--- @class Tooltip
--- @field heading string
--- @field body TooltipBody

local ERROR_SYNTAX_TOOLTIP = {
	heading = "Tooltip Syntax Error",
	body = {
		"Syntax error in tooltip definition in core.lua:",
...
local function GetNotFoundError(name)
	...
	local body = {
		{ ERROR_NOT_FOUND_TOOLTIP.body:format(name), 1.0, 0.0, 0.0 }
	}
	...
	if not hasError then
		for i = 1, #body do
			...
			if not hasError and type(line) ~= "table" then
				heading, body = GetSyntaxError(
					"body line is not a table",
					name,
					("Line %d of the tooltip's body must be a table."):format(i)
				)
			...
			if not hasError and not text then
				heading, body = GetSyntaxError(
					"line text not found",
					name,
					("Line %d of the tooltip's body is missing text."):format(i)
				)
			...
			if not hasError and red and (not green or not blue) then
				heading, body = GetSyntaxError(
					"colour value(s) missing",
					name,
					("Line %d of the tooltip's body must either specify all 3 colour components or none."):format(i)
				)
			...
			if not hasError and red then
				if type(red) ~= "number" or red < 0.0 or red > 1.0 then
					heading, body = GetSyntaxError(
						...
						name,
						(
							"Line %d of the tooltip's body: red value must be a number between 0.0 and 1.0 inclusive."
						):format(i)
					)
				...
				if type(green) ~= "number" or green < 0.0 or green > 1.0 then
					heading, body = GetSyntaxError(
						...
						name,
						(
							"Line %d of the tooltip's body: green value must be a number between 0.0 and 1.0 inclusive."
						):format(i)
					)
				...
				if type(blue) ~= "number" or blue < 0.0 or blue > 1.0 then
					heading, body = GetSyntaxError(
						...
						name,
						(
							"Line %d of the tooltip's body: blue value must be a number between 0.0 and 1.0 inclusive."
						):format(i)
					)
...
for name, data in pairs(assert(oldTooltips)) do
...
--- @return boolean success true if the tooltip was found; false otherwise
--- @return string heading The heading of the tooltip
--- @return Tooltip body The body of the tooltip (a table of line definitions)
...
--- @param definition string The text following the #tooltipdesc metacommand
--- @return boolean success  true if a valid tooltip was found; false otherwise
--- @return string heading The heading of the tooltip
--- @return TooltipBody body The body of the tooltip (a table of line definitions)
function CustomTooltips.GetInlineTooltip(definition)
	...
	if heading and body then
		...
		return false,
			GetSyntaxError(
				"Bad inline tooltip definition format", "<inline>",
				"Macro inline tooltip must follow this format: #tooltipdesc heading^body"
			)
	end
end

--- Populates a GameTooltip with the specified tooltip text.
--- @param tooltipFrame GameTooltip The GameTooltip to populate
--- @param heading string The heading of the tooltip
--- @param body TooltipBody The body of the tooltip (a table of line definitions)
...
--- @param button Button The button to anchor the tooltip to, or nil if the tooltip shouldn't be anchored to a button
--- @param macroText string The macro text
